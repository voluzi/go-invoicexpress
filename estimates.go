...
// to the finalized state, mirroring InvoicesService.CreateAndFinalize.
func (s *EstimatesService) CreateAndFinalize(ctx context.Context, docType DocumentType, req *InvoiceCreateRequest) (*Estimate, error) {
	est, err := s.Create(ctx, docType, req)
	if err != nil {
		return nil, err
	}
	finalized, err := s.ChangeState(ctx, docType, est.ID, StateFinalized, "")
	if err != nil {
