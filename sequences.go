...
// List returns all sequences.
func (s *SequencesService) List(ctx context.Context) ([]Sequence, error) {
	var resp sequenceListResponse
	if err := s.client.do(ctx, http.MethodGet, "/sequences.json", nil, nil, &resp); err != nil {
		return nil, fmt.Errorf("invoicexpress: sequences.list: %w", err)
	}
